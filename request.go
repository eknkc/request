...
// Session is an active HTTP request instance
// Almost all methods are chainable and upon constructing the request you need to call EndXXX variants to actually initiate the request
// Any error occured before End (such as JSON method producing an error) would be returned from EndXXX methods
...
// Errors is a  collection of errors occured during a request session
// A session might generate more than one error during execution, they will be merged into a single error
type Errors []error
...
// Each Client instance has an underlying http.Client instance so try to create one and reuse it.
func New() Client {
	c := client{
